...
export const createDonThuoc = async (req, res, next) => {
  try {
    ...
    const [result] = await pool.execute(
      'INSERT INTO don_thuoc (id_benh_nhan, mo_ta, ngay_ke) VALUES (?, ?, ?)',
      [id_benh_nhan, mo_ta, ngay_ke || new Date()]
    );

    // Insert medicines
    for (const item of thuoc) {
      await pool.execute(
        'INSERT INTO thuoc_trong_don (id_don_thuoc, ten_thuoc, lieu_luong, thoi_diem_uong, ghi_chu) VALUES (?, ?, ?, ?, ?)',
        [result.insertId, item.ten_thuoc, item.lieu_luong, item.thoi_diem_uong, item.ghi_chu]
      );
    }
  ...
  try {
    ...
    if (thuoc && Array.isArray(thuoc)) {
      ...
      for (const item of thuoc) {
        await pool.execute(
          'INSERT INTO thuoc_trong_don (id_don_thuoc, ten_thuoc, lieu_luong, thoi_diem_uong, ghi_chu) VALUES (?, ?, ?, ?, ?)',
          [id, item.ten_thuoc, item.lieu_luong, item.thoi_diem_uong, item.ghi_chu]
        );
      }
    }
