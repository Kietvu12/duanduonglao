...
export const getAllBaiViet = async (req, res, next) => {
  try {
    const { page = 1, limit = 10, search, category, trang_thai } = req.query;
    ...
    query += ' ORDER BY bv.ngay_dang DESC, bv.ngay_tao DESC LIMIT ? OFFSET ?';
...
export const createBaiViet = async (req, res, next) => {
  try {
    const {
      ...
      category, tags, trang_thai, ngay_dang
    } = req.body;
    ...
    const [result] = await pool.execute(
      `INSERT INTO bai_viet 
       (tieu_de, slug, noi_dung, anh_dai_dien, meta_title, meta_description, 
        category, tags, trang_thai, ngay_dang, id_tac_gia)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        tieu_de || null,
        finalSlug || null,
