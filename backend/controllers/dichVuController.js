...
export const getAllDichVu = async (req, res, next) => {
  try {
    ...
    query += ' ORDER BY dv.ngay_tao DESC LIMIT ? OFFSET ?';
...
export const createDichVu = async (req, res, next) => {
  try {
    const { ten_dich_vu, mo_ta_ngan, mo_ta_day_du, anh_dai_dien, gia_thang, gia_quy, gia_nam } = req.body;
    ...
    const [result] = await pool.execute(
      `INSERT INTO dich_vu (ten_dich_vu, mo_ta_ngan, mo_ta_day_du, anh_dai_dien)
       VALUES (?, ?, ?, ?)`,
      [
        ten_dich_vu,
  ...
  try {
    ...
    if (gia_thang !== undefined || gia_quy !== undefined || gia_nam !== undefined) {
      ...
      if (existingPrice.length > 0) {
        await pool.execute(
          'UPDATE bang_gia_dich_vu SET gia_thang = ?, gia_quy = ?, gia_nam = ? WHERE id_dich_vu = ?',
          [
            sanitizeValue(gia_thang),
            sanitizeValue(gia_quy),
            sanitizeValue(gia_nam),
        ...
        await pool.execute(
          'INSERT INTO bang_gia_dich_vu (id_dich_vu, gia_thang, gia_quy, gia_nam) VALUES (?, ?, ?, ?)',
          [
            id,
            sanitizeValue(gia_thang),
            sanitizeValue(gia_quy),
