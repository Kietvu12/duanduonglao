...
const VRView = ({ onClose }) => {
  ...
  useEffect(() => {
    ...
    const updateCameraRotation = () => {
      ...
      const x = Math.cos(rotation.current.phi) * Math.sin(rotation.current.theta);
      const y = Math.sin(rotation.current.phi);
      const z = Math.cos(rotation.current.phi) * Math.cos(rotation.current.theta);
    ...
    const handleMouseMove = (e) => {
      ...
      rotation.current.phi = Math.max(-Math.PI / 2, Math.min(Math.PI / 2, rotation.current.phi));
